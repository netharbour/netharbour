...
if (defined $ifdescr || defined $iftype) {
	...
	if (defined $iftype) {
		...
		$ifdescr =~ s/\//\\\//g;
		if (fetch_ifindex_from_cmdb($hostname,$ifdescr)) {
			# This means we found the key in database.
			# so we don't need to query the device for it
			$status =$snmpkey;
		} else {
			# else query device
...
#print ("$state: $answer| percent_packet_loss=10ms, rta=0.80c \n");
print ("$state: $answer\n");
exit $ERRORS{$state};
...
sub fetch_ifindex_from_cmdb {
	my $device_fqdn = shift;
	my $interface_name = shift;
	my  $query = "Select interfaces.disc_interface_index 
		FROM interfaces, Devices
		WHERE
			Devices.device_fqdn = '$device_fqdn'
			AND Devices.device_id = interfaces.interface_device
			AND (interfaces.interface_descr = '$interface_name' OR interfaces.interface_name = '$interface_name')
			AND interfaces.active = '1'
		";
	...
	if ($sth->rows == 0) {
		...
		if ($sth->rows > 1) {
			warn "Found multiple interfaced indexes for $device_fqdn $interface_name\n";
		}
