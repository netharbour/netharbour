...
sub analyze_temperature {
	...
	if ((!defined($snmp_result->{$temp_oid_re1})) || ($snmp_result->{$temp_oid_re1} eq "noSuchInstance")) {
...
sub analyze_memory_usage {
	...
	if ((!defined($snmp_result->{$mem_oid_re1})) || ($snmp_result->{$mem_oid_re1} eq "noSuchInstance")) {
...
sub analyze_cpu {
	...
	if ((!defined($snmp_result->{$cpu_oid_re1})) || ($snmp_result->{$cpu_oid_re1} eq "noSuchInstance")) {
	...
	if ((!defined($snmp_result->{$cpu_redundancy_state_oid_re0})) || ($snmp_result->{$cpu_redundancy_state_oid_re0} eq "noSuchInstance")) {
		$cpu_status_str_1 .=  "Could not determine redundance state for for re0 ";
		$cpu_status_exit_1 =  1;
